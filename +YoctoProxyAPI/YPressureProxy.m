% YPressureProxy: pressure sensor control interface, available for instance in the
...
classdef YPressureProxy < YoctoProxyAPI.YSensorProxy
    % YPressureProxy: pressure sensor control interface, available for instance in the
    % Yocto-Altimeter-V2, the Yocto-CO2-V2, the Yocto-Meteo-V2 or the Yocto-Pressure
    % 
    % The YPressureProxy class allows you to read and configure Yoctopuce pressure sensors. It inherits
    ...
    methods (Hidden)
        function obj = YPressureProxy()
            % YPressureProxy For internal use as System Object.
            % Use FindPressure() instead.
            obj = obj@YoctoProxyAPI.YSensorProxy();
            obj.classHandle = -1495678415;
            obj.numOutputs = 1;
        end
    end

    methods (Static)
        function obj = FindPressure(func)
            % FindPressure Retrieve instances of YPressureProxy
            obj = YoctoProxyAPI.YPressureProxy;
    ...
    methods (Static)
        function list = GetSimilarFunctions()
            ...
            %
            % Each of these IDs can be provided as argument to the method FindPressure to obtain an
            % object that can control the corresponding device.
            list = YoctoProxyAPI.YFunctionProxy.YpaGetSimilarFunctions(-1495678415);
        end
    end
